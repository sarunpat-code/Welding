        ...
        try:
            ...
            if len(results) > 0:
                ...
                if boxes is not None:
                    for box in boxes:
                        # ultralytics box values are often nested [[x1,y1,x2,y2]] inside list
                        xyxy = box.xyxy.tolist()
                        if isinstance(xyxy, list) and len(xyxy) == 1 and isinstance(xyxy[0], list):
                            xyxy = xyxy[0]

                        # Ensure we have 4 coords
                        if not (isinstance(xyxy, list) and len(xyxy) == 4):
                            app.logger.warning("Unexpected xyxy format: %s", xyxy)
                            continue

                        conf_raw = box.conf.tolist()
                        if isinstance(conf_raw, list) and len(conf_raw) == 1:
                            conf_raw = conf_raw[0]
                        confidence_val = float(conf_raw)

                        cls_raw = box.cls.tolist()
                        if isinstance(cls_raw, list) and len(cls_raw) == 1:
                            cls_raw = cls_raw[0]
                        cls_id = int(cls_raw)
            ...
            if detections:
                app.logger.debug("Detected %d boxes: %s", len(detections), detections)
...
def inspection_history():
    """Show inspection history with advanced filtering options."""
    db = get_db()

    # Get filter parameters
    product_code = request.args.get('product_code', '').strip()
    lot_number = request.args.get('lot_number', '').strip()
    inspector_name = request.args.get('inspector_name', '').strip()
    ...
    inspectors = db.execute("SELECT DISTINCT inspector_name FROM inspections ORDER BY inspector_name").fetchall()
    products = db.execute("SELECT DISTINCT product_code FROM inspections ORDER BY product_code").fetchall()
    lots = db.execute("SELECT DISTINCT lot_number FROM inspections ORDER BY lot_number").fetchall()
    defect_types = db.execute("SELECT DISTINCT defect_type FROM inspections WHERE defect_type IS NOT NULL AND defect_type != 'None' ORDER BY defect_type").fetchall()

    # Get statistics
    total_count = db.execute("SELECT COUNT(*) FROM inspections").fetchone()[0]
    ok_count = db.execute("SELECT COUNT(*) FROM inspections WHERE result = 'OK'").fetchone()[0]
    ng_count = db.execute("SELECT COUNT(*) FROM inspections WHERE result = 'NG'").fetchone()[0]
